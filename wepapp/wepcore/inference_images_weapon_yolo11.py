...
if framework == 'tflite':
    ...
    saved_model_loaded_weapon = YOLO(weights_weapon)
    #tf.saved_model.load(weights_weapon, tags=[tag_constants.SERVING])
    log.info("loaded weapon model")
...
def inference_images_weapon(image_mask1,video_name,frame_num):
    ...
    try:
        ...
        image_data = image_mask1#cv2.resize(image_mask1, (input_size_weapon, input_size_weapon))
        ...
        if framework == 'tflite':
            ...
            log.info("inference started here")
            #batch_data = image_data.squeeze(0)
            pred_bbox = infer_weapon(image_data) #inference happens hrer
            end_time = time.time()
        ...
        for result in pred_bbox:
            ...
            classes = [result.names[cls.item()] for cls in result.boxes.cls.int()]  # class name of each box
            scores = result.boxes.conf  # confidence score of each box
            image2 = result.plot()

        original_h, original_w, _ = image_mask1.shape
        bboxes = utils.format_boxes(xyxy.cpu().numpy(), original_h, original_w)

        # hold all detection data in one variable
        pred_bbox = [bboxes, scores.numpy(), np.array(classes), np.ones(len(classes))]
