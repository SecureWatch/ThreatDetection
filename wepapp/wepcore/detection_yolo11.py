...
def detect(msg_py,msg_local_py,msg_passing):
        from wepcore.inference_images_weapon_yolo11 import inference_images_weapon
        ...
        if msg_local_py is None:
            ...
            try:
                ...
                while keep_trying == True:
                        ...
                        # if the array are empty inset 0 
                        if (len(scores_weapon)==0):
                            np.insert(scores_weapon,0,0)
                            np.insert(classes_weapon,0,0)

                        image_capture_path = ""
                        weapon_threats_in_frame = []
                    
                    #weapon json to show frames where weapons detected
                        for s,c in zip(scores_weapon,classes_weapon):
                            if (float(s)>threshold):
                                if ((c ==b'rifle') or (c == b'gun') or (c == b'Handgun')):
                                    ...
                                    detection_class  = 'Person : '+str(s)  #Added score for prediction also
                                else:
